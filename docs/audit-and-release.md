...
`jobs:self-healing -- --all --replace` regenerates bounded live job specs and generated context packs for the full current triage-state recommended-job queue, replacing stale specs when a promotion or canonical change changes the queue.
