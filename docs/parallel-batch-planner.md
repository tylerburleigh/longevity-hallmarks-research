...
Command-event caps are disabled. Keep jobs bounded with context packs, read/write sets, timeout guards, and focused prompts rather than per-worker shell command counts.
